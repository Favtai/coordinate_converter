...
source_crs = col1.selectbox("Select Source CRS:", options=list(epsg_dict.keys()))
target_crs = col2.selectbox("Select Target CRS:", options=list(epsg_dict.keys()))
st.text(f"Source CRS: {epsg_dict[source_crs]} | Target CRS: {epsg_dict[target_crs]}")

# --- Coordinate Conversion Function ---
def coordinate_conv(lon, lat, source_crs, target_crs, epsg_dict):
    """
    Converts coordinates from source CRS to target CRS using pyproj.
    Accepts scalars or pandas Series for lon and lat.
    """
    source_crs = pyproj.CRS(epsg_dict[source_crs])
    target_crs = pyproj.CRS(epsg_dict[target_crs])
...
if not batch_mode:
    ...
    lon= col3.number_input("Enter Longitude (Eastings or X):", format="%0.4f")
    lat= col4.number_input("Enter Latitude (Northings or Y):", format="%0.4f")
    ...
    if st.session_state.get("convert_btn"):
        ...
        if source_crs and target_crs:
            try:
                ...
                x, y = coordinate_conv(lon, lat, source_crs, target_crs, epsg_dict)
                ...
                to_wgs84 = pyproj.Transformer.from_crs(pyproj.CRS(epsg_dict[target_crs]), wgs84, always_xy=True)
...
# --- Batch (CSV) Conversion Mode ---
    ...
    if csv_path is not None:
        ...
        if lon_col is None or lat_col is None:
            ...
            df['CONVERTED_X'], df['CONVERTED_Y'] = coordinate_conv(df[lon_col], df[lat_col], source_crs, target_crs, epsg_dict)
